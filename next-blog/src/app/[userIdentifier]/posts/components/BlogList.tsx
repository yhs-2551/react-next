...
function BlogList({ initialData, userIdentifier }: BlogListProps) {
    const [userIdentifierFromToken, setUserIdentifierFromToken] = useState<string | null>(null);
    const posts = initialData;
 

    const TOKEN_KEY = "access_token"; // Single source of truth for token key
    ...
    useEffect(() => {
        try {
            ...
            if (token) {
                ...
                setUserIdentifierFromToken(decodedToken.userIdentifier);
            }
        } catch (error) {
            console.error("Error decoding token:", error);
        }

        console.log("userIdentifierFromToken >>>", userIdentifierFromToken);
        ...
        if (!isValidToken) {
            ...
            if (newAccessToken) {
                ...
                if (isValidToken) {
                    ...
                    window.location.assign(`/${userIdentifier}/posts/new`);
    ...
    return (
        <>
            ...
            <div className='container max-w-4xl mx-auto p-6'>
                ...
                {userIdentifier === userIdentifierFromToken && <button onClick={handleNewPost}>글쓰기</button>}
            </div>
        </>
    );
}
