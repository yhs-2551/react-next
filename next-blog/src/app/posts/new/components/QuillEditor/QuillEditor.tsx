        ...
        const handleImageClick = useCallback((event: MouseEvent) => {
            ...
            if ((event.target as HTMLElement).tagName === "IMG") {
                ...
                if (
                    ...
                    console.log("sleectedImage >>>", imgEl);
            ...
            const handleResizeStart = (
                    ...
                    if (
                        ...
                        // 중앙 정렬 상태에서 양쪽 핸들러가 컨테이너를 벗어나지 않도록 제한
                            ...
                            const imageRightEdge =
                                selectedImage.offsetLeft +
                                selectedImage.offsetWidth; // 이미지 오른쪽 끝 좌표 계산
                    ...
                    if (newWidth > 100 && newHeight > 100) {
                        ...
                        if (overlay) {
                            ...
                            if (
                                ...
                                const rightEdge =
                                    containerRect.width - newWidth; // 오른쪽 끝에서 시작하는 위치 계산
                ...
                const handleResizeEnd = () => {
                    ...
                    if (overlay && selectedImage) {
                        ...
                        const quill = quillRef.current?.getEditor();
                        if (overlay && selectedImage && quill) {
                            selectedImage.style.width = overlay.style.width;
                            selectedImage.style.height = overlay.style.height;
                            selectedImage.style.opacity = "1";

                            const id = selectedImage.getAttribute("data-id");
                                ...
                                const updatedOps = delta.ops.map((op) => {
                                    if (
                                        ...
                                        return {
                                            ...
                                            attributes: {
                                                ...op.attributes,
                                                width: selectedImage.style
                                                    .width,
                                                height: selectedImage.style
                                                    .height,
                                            },
                                        };
                                    }
                                    return op; // Return the original operation for others
                                });

                                // Now apply the updated ops back to the Quill editor
                                quill.setContents(
                                    new Delta({ ops: updatedOps })
                                );
                            }
                        }
                    }
                            ...
                            if (type === "image") {
                                if (savedSelection === 0) {
                                    ...
                                    quill.insertEmbed(
                                        ...
                                        {
                                            alt: "Quill Cloud",
                                            url: "https://quilljs.com/0.20sdadag",
                                        },
                                        Quill.sources.USER
                                    ); // 이미지 삽입

                                    // 커서가 index 0일 때 처리 로직
                                    // quill.insertEmbed(savedSelection, "image", {
                                    //     url: base64String,
                                    //     width: "auto",
                                    //     height: "auto",
                                    //     alt: "하이하이",
                                    ...
                                    // Use a small delay to ensure the image is inserted into the DOM before applying the ID
                                    // setTimeout(() => {
                                    //     // Find the newly inserted image in the editor
                                    //     const imgElements =
                                    //         quill.root.querySelectorAll("img");
                                    //     const insertedImg =
                                    //         imgElements[imgElements.length - 1]; // Get the last inserted image

                                    //     if (insertedImg) {
                                    //         console.log("fff", insertedImg);

                                    //         // Manually set the custom `id` attribute
                                    //         insertedImg.setAttribute(
                                    //             "id",
                                    //             uniqueId
                                    //         );
                                    //         console.log(
                                    //             `Image ID set to: ${uniqueId}`
                                    //         );
                                    //     }
                                    // }, 500);
                                    quill.setSelection(
        ...
        return (
            <>
                ...
                <ReactQuillDynamic
                    ...
                    theme='snow'
                    modules={modules}
                    formats={["image", "alt", "url", "width", "height"]}
                />
