...
class CustomImageBlot extends BlockEmbed {
    ...
    static tagName = 'div';  // 기존 'img'에서 'div'로 변경

    static create(value: any): Node {
        const node = super.create(value);
        const child = document.createElement('img');  // 'img' 태그 생성
        child.setAttribute('src', value.src);  // 이미지 URL 설정
        if (value.id) {
            child.setAttribute('id', value.id);  // 이미지 ID 설정
        }
        child.setAttribute('width', value.width || 'auto');  // 이미지 width 설정
        child.setAttribute('height', value.height || 'auto');  // height 설정 (필요 시)
        node.appendChild(child);  // img 태그를 div 내부에 추가
        return node;
    }

    static value(node: any): any {
        const img = node.querySelector('img');  // div 내의 img 태그 선택
        return {
            src: img?.getAttribute('src'),  // 이미지 src 반환
            id: img?.getAttribute('id'),  // 이미지 id 반환
            width: img?.getAttribute('width'),  // 이미지 width 반환
            height: img?.getAttribute('height')  // 이미지 height 반환
        };
    }
}
Quill.register({
...
export default React.memo(
  React.forwardRef<ReactQuill, QuillEditorProps>(function QuillEditor(
    ...
    const [base64String, setBase64String] = useState<string | undefined>(""); // Base64 상태 관리
    ...
    let selectedImageIndex: number | undefined = undefined; // 선택된 이미지의 range를 저장할 변수
        ...
        if (target) {
          ...
          delta.ops.forEach((op, index) => {
            if (op.insert && typeof op.insert === "object") {
              console.log("실행");
              const imageObj: any = op.insert;

              console.log("imageObjID >>>", imageObj);
              console.log("targetId >>>" + targetId);

              if (imageObj.id === targetId) {
                console.log("index >>>" + index);

                selectedImageIndex = index; // 현재 인덱스를 저장
            ...
            if (
              ...
              if (type === "image") {
                if (savedSelection === 0) {
                  ...
                  setTimeout(() => {
                    const imgElement =
                      quill.root.querySelectorAll("img")[savedSelection!]; // index로 해당 위치의 이미지를 가져옴
                    if (imgElement) {
                      imgElement.setAttribute("id", uniqueId); // img 태그에 고유 ID 속성 추가
                    }
                  }, 0); // 약간의 지연을 줘서 이미지가 DOM에 완전히 반영된 후 작업

                  const delta = quill.getContents(); // Delta에서 전체 내용 가져오기
