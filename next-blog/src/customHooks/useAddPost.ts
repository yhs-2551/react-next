...
function useAddPost(userIdentifier: string) {
    ...
    return useMutation(async (newPost: PostRequest) => {
        ...
        const createPost: (token: string | boolean) => Promise<Response> = async (token: string | boolean) => {
            return await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}${process.env.NEXT_PUBLIC_BACKEND_PATH}/${userIdentifier}/posts`, {
                method: "POST",
                headers: {
